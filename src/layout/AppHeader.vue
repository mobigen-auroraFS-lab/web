<template>
  ...
  >
    <div class="flex items-center gap-8 min-w-0">
      <a
        href="#file-search"
        class="flex items-center shrink-0 rounded-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus-ring"
      >
      ...
      <nav class="flex items-center gap-1">
        <a
          ...
          :class="
            active === item.key
              ? 'font-semibold text-text-primary bg-slate-100'
              : 'font-medium text-text-secondary hover:bg-bg-surface-hover'
          "
          >{{ item.label }}</a
        >
      </nav>
    ...
    <div class="flex items-center gap-3">
      ...
      <button
        title="로그아웃"
        aria-label="로그아웃"
        class="w-8 h-8 flex items-center justify-center bg-transparent border border-border-default rounded-md text-icon-default cursor-pointer hover:bg-bg-surface-hover hover:border-border-strong hover:text-text-primary"
      >
